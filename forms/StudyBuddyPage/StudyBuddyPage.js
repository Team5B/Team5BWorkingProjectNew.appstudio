...
btnFindSB.onclick=function(){                                                                                                                                                       
  query = "SELECT * FROM `table` t JOIN `user_table` ut ON t.table_id = ut.table_id JOIN `user` u ON u.user_id = ut.user_id WHERE `open` = 1 AND `first_name` = '" + inpFirstNameSB + "' AND `last_name = '" + inpLastNameSB + "' " "
